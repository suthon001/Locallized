/// <summary>
/// Report NCT Fixed Asset - Pro. Value (ID 80078).
/// </summary>
report 80078 "NCT Fixed Asset - Pro. Value"
{
    DefaultLayout = RDLC;
    RDLCLayout = './LayoutReport/StandardReport/Report_80078_FixedAssetProjectedValue.rdl';
    ...
    var
        ...
        CalculateDepr: Codeunit "NCT Calculate Depreciation";
        FADateCalculation: Codeunit "FA Date Calculation";
        DepreciationCalculation: Codeunit "NCT Depreciation Calculation";
        FAFilter: Text;
        DeprBookText: Text;
        GroupCodeName: Text;
        GroupCodeName2: Text;
        ...
        Year365Days, Year366Days : Boolean;
    ...
    begin
        ...
        EntryAmounts[3] := DepreciationCalculation."NCT DeprInFiscalYear"("Fixed Asset"."No.", DeprBookCode, StartingDate);
        ...
        EndFiscalYear := FADateCalculation.CalculateDate(
            DepreciationCalculation."NCT Yesterday"(NewFiscalYear, Year365Days, Year366Days), DaysInFiscalYear, Year365Days);
    ...
    begin
        ...
        EntryAmounts[3] :=
          DepreciationCalculation."NCT DeprInFiscalYear"("Fixed Asset"."No.", DeprBookCode, UntilDate);
    end;
    ...
    begin
        ...
        if UntilDate = EndFiscalYear then begin
            EntryAmounts[3] := 0;
            NewFiscalYear := DepreciationCalculation."NCT ToMorrow"(EndFiscalYear, Year365Days, Year366Days);
            EndFiscalYear := FADateCalculation.CalculateDate(EndFiscalYear, DaysInFiscalYear, Year365Days);
        end;

        DateFromProjection := DepreciationCalculation."NCT ToMorrow"(UntilDate, Year365Days, Year366Days);
    ...
    begin
        ...
        AccountingPeriod.SetFilter(
          "Starting Date", '>=%1', DepreciationCalculation."NCT ToMorrow"(PeriodEndingDate, Year365Days, Year366Days) + 1);
        if AccountingPeriod.FindFirst() then begin
            if Date2DMY(AccountingPeriod."Starting Date", 1) <> 31 then
                UntilDate2 := DepreciationCalculation."NCT Yesterday"(AccountingPeriod."Starting Date", Year365Days, Year366Days)
            else
                UntilDate2 := AccountingPeriod."Starting Date" - 1;
            pPeriodLength :=
              DepreciationCalculation."NCT DeprDays"(
                DepreciationCalculation."NCT ToMorrow"(PeriodEndingDate, Year365Days, Year366Days), UntilDate2, Year365Days, Year366Days);
            if (pPeriodLength <= 5) or (pPeriodLength > DaysInFiscalYear) then
                pPeriodLength := DaysInFiscalYear;
            exit(UntilDate2);
        end;
        if Year365Days then
            Error(YouMustCreateAccPeriodsErr, DepreciationCalculation."NCT ToMorrow"(EndingDate, Year365Days, Year366Days) + 1);
        exit(FADateCalculation.CalculateDate(PeriodEndingDate, pPeriodLength, Year365Days));
    end;
    ...
    var
        ...
        ltEntryAmounts: array[14] of Decimal;
        PrevAmount: array[2] of Decimal;
    begin
        PrevAmount[1] := AssetAmounts[3];
        PrevAmount[2] := AssetAmounts[4];

        CalculateDisposal.CalcGainLoss("Fixed Asset"."No.", DeprBookCode, ltEntryAmounts);
        AssetAmounts[3] := FADeprBook."Projected Proceeds on Disposal";
        if ltEntryAmounts[1] <> 0 then
            AssetAmounts[4] := ltEntryAmounts[1]
        else
            AssetAmounts[4] := ltEntryAmounts[2];
        AssetAmounts[4] :=
    ...
    var
        ltDeprBook: Record "Depreciation Book";
    begin
        if DeprBookCode <> '' then
            ltDeprBook.Get(DeprBookCode);
        PeriodLength := 0;
        if ltDeprBook."Fiscal Year 365 Days" and not UseAccountingPeriod then
            PeriodLength := 365;
    end;
    ...
    /// SetPeriodFields.
    /// </summary>
    /// <param name="PeriodLengthFrom">Integer.</param>
    /// <param name="DaysInFirstPeriodFrom">Integer.</param>
    /// <param name="IncludePostedFromFrom">Date.</param>
    /// <param name="UseAccountingPeriodFrom">Boolean.</param>
    ...
    /// SetBudgetField.
    /// </summary>
    /// <param name="BudgetNameCodeFrom">Code[10].</param>
    /// <param name="BalAccountFrom">Boolean.</param>
    /// <param name="ProjectedDisposalFrom">Boolean.</param>
    /// <param name="PrintAmountsPerDateFrom">Boolean.</param>
