...
codeunit 80003 "NCT Get Cust/Vend Ledger Entry"
{
        ...
        CASE BillingHeader."Document Type" OF
            BillingHeader."Document Type"::"Sales Billing", BillingHeader."Document Type"::"Sales Receipt":
                BEGIN
                    ...
                    IF BillingHeader."Document Type" = BillingHeader."Document Type"::"Sales Billing" THEN
                        CustLedgEntry.SETFILTER("NCT Completed Billing", '%1', FALSE)
                    ELSE
                        CustLedgEntry.SETFILTER("NCT Completed Receipt", '%1', FALSE);
            ...
            BillingHeader."Document Type"::"Purchase Billing":
                BEGIN
                    ...
                    VendLedgEntry.SETFILTER("NCT Completed Billing", '%1', FALSE);
    ...
    begin
        ...
        IF CustLedgEntry2.FINDSET() THEN
            REPEAT
                ...
                CustLedgEntry2.CALCFIELDS("Original Amount", "Original Amt. (LCY)", "NCT Billing Amount", "Billing Amount (LCY)",
                 "NCT Receipt Amount", "NCT Receipt Amount (LCY)");
                ...
                CASE BillingLine."Document Type" OF
                    BillingLine."Document Type"::"Sales Billing":
                        BEGIN
                            BillingLine."Amount" := CustLedgEntry2."Original Amount" - CustLedgEntry2."NCT Billing Amount";
                            ...
                            CustLedgEntry2."NCT Completed Billing" := TRUE;
                            CustLedgEntry2.MODIFY();
                        END;
                    BillingLine."Document Type"::"Sales Receipt":
                        BEGIN
                            BillingLine."Amount" := CustLedgEntry2."Original Amount" - CustLedgEntry2."NCT Receipt Amount";
                            // CurrencyFactor := BillingLine."Source Amount" / BillingLine."Source Amount (LCY)";
                            if CustLedgEntry2."Currency Code" <> '' then
                                BillingLine."Amount (LCY)" := BillingLine."Amount" * CurrencyFactor
                            else
                            ...
                            CustLedgEntry2."NCT Completed Receipt" := TRUE;
                            CustLedgEntry2.MODIFY();
                        END;
                END;
                BillingLine.MODIFY();
    ...
    begin
        ...
        IF VendLedgEntry2.FINDSET() THEN
            REPEAT
                CurrencyFactor := 0;
                VendLedgEntry2.CALCFIELDS("Original Amount", "Original Amt. (LCY)", "NCT Billing Amount", "Billing Amount (LCY)");
                ...
                BillingLine."Amount" := -VendLedgEntry2."Original Amount" - VendLedgEntry2."NCT Billing Amount";
                BillingLine."Source Currency Factor" := CurrencyFactor;
                if VendLedgEntry2."Currency Code" <> '' then
                    BillingLine."Amount (LCY)" := BillingLine."Amount" * CurrencyFactor
                else
                    BillingLine."Amount (LCY)" := BillingLine."Amount";
                VendLedgEntry2."NCT Completed Billing" := TRUE;
